import { FormatEnum, LeaderEnum } from "../tournament.enum";

export const UP_REIJU_RANKED = [
    {
        leader: LeaderEnum.UP_REIJU,
        opponents: [
             //0-1
            { leader: LeaderEnum.RP_LAW, wins: 0, loses: 1 },
            //1-0
            { leader: LeaderEnum.BLACK_MORIA, wins: 1, loses: 0 }, 
            { leader: LeaderEnum.RY_SABO, wins: 1, loses: 0 }, 
            { leader: LeaderEnum.UB_SAKAZUKI, wins: 1, loses: 0 }, 
            //1-1
            { leader: LeaderEnum.YELLOW_KATAKURI, wins: 1, loses: 1 }, 
            { leader: LeaderEnum.BY_LUFFY, wins: 1, loses: 1 }, 
            { leader: LeaderEnum.GY_YAMATO, wins: 1, loses: 1 }, 
        ],
        format: FormatEnum.OP06
    },
    {
        leader: LeaderEnum.UP_REIJU,
        opponents: [
            //2-0
            { leader: LeaderEnum.GB_PERONA, wins: 2, loses: 0 }, 
            //1-0
            { leader: LeaderEnum.RP_LAW, wins: 1, loses: 0 }, 
            //1-1
            { leader: LeaderEnum.BLUE_BOA, wins: 1, loses: 1 }, 
            //2-2
            { leader: LeaderEnum.YELLOW_ENEL, wins: 2, loses: 2 }, 
            //2-1
            { leader: LeaderEnum.BLACK_MORIA, wins: 2, loses: 1 }, 
            //3-1
            { leader: LeaderEnum.BLACK_LUCCI_OP07, wins: 3, loses: 1 }, 
            { leader: LeaderEnum.GREEN_BONNEY, wins: 3, loses: 2 }, 
            { leader: LeaderEnum.BLUE_NAMI, wins: 3, loses: 2 }, 
            { leader: LeaderEnum.BY_LUFFY, wins: 3, loses: 3 }, 
            { leader: LeaderEnum.BLUE_DOFFY, wins: 3, loses: 0 }, 
            { leader: LeaderEnum.UP_REIJU, wins: 3, loses: 0 }, 
        ],
        format: FormatEnum.OP07
    },
    {
        leader: LeaderEnum.UP_REIJU,
        opponents: [
             //0-1
            { leader: LeaderEnum.PURPLE_LUFFY, wins: 0, loses: 1 },
            { leader: LeaderEnum.YELLOW_ENEL, wins: 0, loses: 1 },
            //1-0
            { leader: LeaderEnum.BY_LUFFY, wins: 1, loses: 0 }, 
            //1-1
            { leader: LeaderEnum.BLACK_LUCCI_OP07, wins: 1, loses: 1 }, 
            { leader: LeaderEnum.BLUE_DOFFY, wins: 1, loses: 1 }, 
            { leader: LeaderEnum.BLUE_NAMI, wins: 3, loses: 1 }, 
        ],
        format: FormatEnum.OP08_5
    },
    // {
    //     leader: LeaderEnum.UP_REIJU,
    //     opponents: [
    //          //0-2
    //         { leader: LeaderEnum., wins: 0, loses: 2 },
    //          //0-1
    //         { leader: LeaderEnum., wins: 0, loses: 1 },
    //         //2-0
    //         { leader: LeaderEnum., wins: 2, loses: 0 }, 
    //         //1-0
    //         { leader: LeaderEnum., wins: 1, loses: 0 }, 
    //         //1-2
    //         { leader: LeaderEnum., wins: 1, loses: 2 }, 
    //         //1-1
    //         { leader: LeaderEnum., wins: 1, loses: 1 }, 
    //         //2-2
    //         { leader: LeaderEnum., wins: 2, loses: 2 }, 
    //         //2-1
    //         { leader: LeaderEnum., wins: 2, loses: 1 }, 
    //         //3-1
    //         { leader: LeaderEnum., wins: 3, loses: 1 }, 
    //     ],
    //     format: FormatEnum.OP06
    // },
]